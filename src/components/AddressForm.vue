  ...
  <div>
    ...
    <div v-if="!isAddressSaved">
      <div class="m-auto my-4 border border-2 w-50 m-auto p-4 rounded rounded-4 bg-white">
        <h4 class="py-4" id="address-title">Delivery address</h4>
    ...
    <div v-else-if="isAddressSaved && isEditActive">
      <div id="address-saved-form" class="px-5 pt-3 m-auto mt-4 border border-2 w-50 m-auto rounded rounded-4">
        ...
        <div class="delete-div d-inline-block px-2" @click="removeAddress">
        ...
        <div class="edit-card d-inline-block" @click="editAddress">
        ...
        <p class=" px-4 address">{{ deliveryAddress.city }}, {{ deliveryAddress.country }},{{
            deliveryAddress.street
          }},{{ deliveryAddress.number }},
          {{ deliveryAddress.blockStaircase }}</p>
        <p class=" px-4 address" id="postcode">{{ deliveryAddress.postcode }}</p>
      </div>
      <button @click="updateAddress(deliveryAddress, index)" class="btn btn-primary mb-5">Save edit</button>
    </div>

    <div v-else-if="isAddressSaved && !isEditActive" id="address-form">
      <div id="address-saved-form" class="px-5 pt-3 m-auto mt-4 border border-2 w-50 m-auto rounded rounded-4">
        <h5 class=" px-4 my-address"> Delivery address </h5>
        <div class="delete-div d-inline-block px-2" @click="removeAddress">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
...
export default {
  name: "AddressForm",
  props: ['isAddressSavedInitial', 'address', 'index'],
  data() {
    return {
  ...
  methods: {
    ...
    saveAddress() {
      ...
      this.isAddressSaved = !this.isCountryIncomplete && !this.isCityIncomplete
